...
# Вывод моделей в удобном формате
print("\n" + "="*50)
print("ДОСТУПНЫЕ МОДЕЛИ:")
for i, model in enumerate(models, 1):
    print(f"{i}. {model}")
...
# Используем только модель openai
selected_model = "openai"

# Спросить, нужно ли загрузить изображение
use_image = input("Хотите загрузить изображение? (y/n): ").lower().strip() == 'y'

image_base64 = None
if use_image:
    print("\n" + "="*50)
    print("Укажите путь к файлу на вашем устройстве (например: image.jpg)")
    ...
    try:
        ...
        print(f"❌ {e}")
        print("Продолжаем без изображения.")
...
# Ввод запроса от пользователя
print("\n" + "="*50)
user_prompt = input("Введите ваш запрос к изображению: ")

# Максимальное количество токенов
try:
    max_tokens = int(input("\nМаксимальное количество токенов [300]: ") or "300")
except ValueError:
    max_tokens = 300

# Системный промпт (опционально)
system_prompt = input("Системный промпт (например: 'Ты эксперт по анализу изображений') [пусто]: ") or None
